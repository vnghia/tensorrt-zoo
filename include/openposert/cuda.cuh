...
// Round functions
// Signed
template <typename T> inline __device__ char positiveCharRoundCuda(const T a) {
  return char(a + 0.5f);
}
...
template <typename T> inline __device__ int positiveIntRoundCuda(const T a) {
  return int(a + 0.5f);
}

template <typename T> inline __device__ long positiveLongRoundCuda(const T a) {
  return long(a + 0.5f);
}
...
template <typename T> inline __device__ unsigned int uIntRoundCuda(const T a) {
...
// Max/min functions
template <class T> inline __device__ T fastMaxCuda(const T a, const T b) {
  return (a > b ? a : b);
}

template <class T> inline __device__ T fastMinCuda(const T a, const T b) {
  return (a < b ? a : b);
}

template <class T>
inline __device__ T fastTruncateCuda(const T value, const T min = 0,
                                     const T max = 1) {
...
cubicSequentialData(int *xIntArray, int *yIntArray, T &dx, T &dy,
                    const T xSource, const T ySource, const int widthSource,
...
template <typename T>
inline __device__ T cubicInterpolate(const T v0, const T v1, const T v2,
  ...
  return (-0.5f * v0 + 1.5f * v1 - 1.5f * v2 + 0.5f * v3) * dx * dx * dx +
         ...
         + v1;
  // return v1 + 0.5f * dx * (v2 - v0 + dx * (2.f * v0 - 5.f * v1 + 4.f * v2 -
  // v3 + dx * (3.f * (v1 - v2) + v3 - v0)));
}

template <typename T>
inline __device__ T bicubicInterpolate(const T *const sourcePtr,
                                       const T xSource, const T ySource,
                                       const int widthSource,
                                       const int heightSource,
  ...
  T dx;
  T dy;
  cubicSequentialData(xIntArray, yIntArray, dx, dy, xSource, ySource,
...
template <typename T>
inline __device__ T bicubicInterpolate(const unsigned char *const sourcePtr,
                                       const T xSource, const T ySource,
                                       const int widthSource,
                                       const int heightSource,
                                       const int widthSourcePtr) {
  ...
  for (unsigned char i = 0; i < 4; i++) {
    ...
    temp[i] = cubicInterpolate(T(sourcePtr[offset + xIntArray[0]]),
                               ...
                               T(sourcePtr[offset + xIntArray[2]]),
                               T(sourcePtr[offset + xIntArray[3]]), dx);
  }
  return cubicInterpolate(temp[0], temp[1], temp[2], temp[3], dy);
}

template <typename T>
inline __device__ T bicubicInterpolate8Times(const T *const sourcePtr,
                                             const T xSource, const T ySource,
                                             const int widthSource,
                                             const int heightSource,
  ...
  const T dx =
      xSource - fastTruncateCuda(int(floor(xSource)), 0, widthSource - 1);
  const T dy =
  ...
  for (unsigned char i = 0; i < 4; i++) {
    ...
    temp[i] =
        cubicInterpolate(sourcePtr[offset], sourcePtr[offset + 1],
...
template <typename T>
inline __device__ T addWeighted(const T value1, const T value2,
                                const T alphaValue2) {
  return (1.f - alphaValue2) * value1 + alphaValue2 * value2;
}

template <typename T>
inline __device__ void addColorWeighted(T &colorR, T &colorG, T &colorB,
                                        const T *const colorToAdd,
                                        const T alphaColorToAdd) {
