...
template <typename T>
__global__ void nmsRegisterKernel(int *kernelPtr, const T *const sourcePtr,
                                  const int w, const int h, const T threshold) {
  ...
  if (0 < x && x < (w - 1) && 0 < y && y < (h - 1)) {
    ...
    if (value > threshold) {
      ...
      if (value > topLeft && value > top && value > topRight && value > left &&
          value > right && value > bottomLeft && value > bottom &&
          value > bottomRight)
...
template <typename T>
__global__ void
writeResultKernel(T *output, const int length, const int *const kernelPtr,
                  const T *const sourcePtr, const int width, const int height,
                  const int maxPeaks, const T offsetX, const T offsetY,
                  const int offsetTarget) {
  ...
  // We need to substract the peak at pixel 0 of the current channel for all
  // values
  if (threadIdx.x == 0)
  ...
  if (globalIdx < length) {
    ...
    if (globalIdx != length - 1) {
      ...
      if (local[threadIdx.x] != local[threadIdx.x + 1]) {
        ...
        if (peakIndex < maxPeaks) // limitation
        {
          ...
          outputOffset[outputIndex + 2] =
              sourcePtrOffset[peakLocY * width + peakLocX] +
              0.03; // no resize, score lower about 0.03 compare to caffe
                    // version.
        }
      }
    }
    ...
    // number of peaks)
      ...
      outputOffset[0] =
          (local[threadIdx.x] < maxPeaks ? local[threadIdx.x] : maxPeaks);
  }
}
...
template void nmsGpu(float *targetPtr, int *kernelPtr,
                     const float *const sourcePtr, const float threshold,
                     const std::array<int, 4> &targetSize,
                     const std::array<int, 4> &sourceSize,
                     const Point<float> &offset);
template void nmsGpu(double *targetPtr, int *kernelPtr,
                     const double *const sourcePtr, const double threshold,
                     const std::array<int, 4> &targetSize,
                     const std::array<int, 4> &sourceSize,
