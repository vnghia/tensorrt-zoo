...
//     T* targetPtr, const T* const sourcePtr, const int widthSource, const int
//     heightSource, const int widthTarget, const int heightTarget)
// {
//     const auto x = (blockIdx.x * blockDim.x) + threadIdx.x;
//     const auto y = (blockIdx.y * blockDim.y) + threadIdx.y;
//     if (x < widthTarget && y < heightTarget)
//     {
//         const T xSource = (x + T(0.5f)) * widthSource / T(widthTarget) -
//         T(0.5f); const T ySource = (y + T(0.5f)) * heightSource /
//         T(heightTarget) - T(0.5f); targetPtr[y*widthTarget+x] =
//         bicubicInterpolate(
//             sourcePtr, xSource, ySource, widthSource, heightSource,
//             widthSource);
//     }
// }
...
//     T* targetPtr, const T* const sourcePtr, const T scaleWidth, const T
//     scaleHeight, const int widthSource, const int heightSource, const int
//     widthTarget, const int heightTarget)
// {
//     const auto x = (blockIdx.x * blockDim.x) + threadIdx.x;
//     const auto y = (blockIdx.y * blockDim.y) + threadIdx.y;
//     const auto channel = (blockIdx.z * blockDim.z) + threadIdx.z;
...
//         const T xSource = (x + T(0.5f)) * widthSource / T(widthTarget) -
//         T(0.5f); const T ySource = (y + T(0.5f)) * heightSource /
//         T(heightTarget) - T(0.5f); const T* const sourcePtrChannel =
//         sourcePtr + channel * sourceArea; targetPtr[channel * targetArea +
//         y*widthTarget+x] += bicubicInterpolate(
//             sourcePtrChannel, xSource, ySource, widthSource, heightSource,
//             widthSource);
//     }
// }
...
//     T* targetPtr, const T* const sourcePtr, const T scaleWidth, const T
//     scaleHeight, const int widthSource, const int heightSource, const int
//     widthTarget, const int heightTarget, const int counter)
// {
//     const auto x = (blockIdx.x * blockDim.x) + threadIdx.x;
//     const auto y = (blockIdx.y * blockDim.y) + threadIdx.y;
//     const auto channel = (blockIdx.z * blockDim.z) + threadIdx.z;
...
//             sourcePtrChannel, xSource, ySource, widthSource, heightSource,
//             widthSource);
//         auto& targetPixel = targetPtr[channel * targetArea +
//         y*widthTarget+x]; targetPixel = (targetPixel + interpolated) /
//         T(counter);
//     }
// }
  ...
  try {
    ...
    if (sourceSizes.size() == 1) {
      ...
      if (targetSize[0] > 1 || num == 1) {
        ...
        // OP_CUDA_PROFILE_INIT(REPS);
        // const dim3 threadsPerBlock{THREADS_PER_BLOCK_1D,
        // THREADS_PER_BLOCK_1D, 1}; const dim3 numBlocks{
        ...
        //     targetPtr, sourcePtrs.at(0), widthSource, heightSource,
        //     widthTarget, heightTarget);
        // OP_CUDA_PROFILE_END(timeNormalize2, 1e3, REPS);

        // Optimized function for 8x resize
        // OP_CUDA_PROFILE_INIT(REPS);
        if (widthTarget / widthSource == 1 &&
            heightTarget / heightSource == 1) {
          ...
          fillKernel<<<numBlocks, threadsPerBlock>>>(targetPtr,
                                                     sourcePtrs.at(0), N);
        } else {
          ...
          resize8TimesKernel<<<numBlocks, threadsPerBlock>>>(
              targetPtr, sourcePtrs.at(0), widthSource, heightSource,
              widthTarget, heightTarget, rescaleFactor);
        }
    ...
    // Multi-scaling merging
      ...
      //     const auto scaleInputToNet = scaleInputToNetInputs[i] /
      //     scaleInputToNetInputs[0]; const auto scaleWidth =
      //     scaleToMainScaleWidth / scaleInputToNet; const auto scaleHeight =
      //     scaleToMainScaleHeight / scaleInputToNet;
      ...
      //                 targetPtr + c * targetChannelOffset, sourcePtrs[i] + c
      //                 * sourceChannelOffset, scaleWidth, scaleHeight,
      //                 currentWidth, currentHeight, widthTarget,
      //                 heightTarget);
      //         }
      //     }
      ...
      //                 targetPtr + c * targetChannelOffset, sourcePtrs[i] + c
      //                 * sourceChannelOffset, scaleWidth, scaleHeight,
      //                 currentWidth, currentHeight, widthTarget, heightTarget,
      ...
      //             targetPtr, sourcePtrs[i], scaleWidth, scaleHeight,
      //             currentWidth, currentHeight, widthTarget, heightTarget);
      //     // Last image --> average all
      //     else
      //         resizeAndAverageKernelOld<<<numBlocks, threadsPerBlock>>>(
