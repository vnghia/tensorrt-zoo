...
//     T* pairScoresPtr, const T* const heatMapPtr, const T* const peaksPtr,
//     const unsigned int* const bodyPartPairsPtr, const unsigned int* const
//     mapIdxPtr, const unsigned int maxPeaks, const int numberBodyPartPairs,
...
//             const T* const bodyPartA = peaksPtr + (3*(partA*(maxPeaks+1) +
//             peakA+1)); const T* const bodyPartB = peaksPtr +
//             (3*(partB*(maxPeaks+1) + peakB+1)); const T* const mapX =
//             heatMapPtr + mapIdxX*heatmapWidth*heatmapHeight; const T* const
//             mapY = heatMapPtr + mapIdxY*heatmapWidth*heatmapHeight;
//             pairScoresPtr[outputIndex] = process(
//                 bodyPartA, bodyPartB, mapX, mapY, heatmapWidth,
//                 heatmapHeight, interThreshold, interMinAboveThreshold,
//                 defaultNmsThreshold);
//         }
//         else
...
void connectBodyPartsGpu(Array<T> &poseKeypoints, Array<T> &poseScores,
  ...
  try {
    ...
    // cudaMemcpy(pairScoresCpu.getPtr(), pairScoresGpuPtr, totalComputations *
    // sizeof(T),
...
inline T getScoreAB(const int i, const int j, const T *const candidateAPtr,
  ...
  try {
    ...
    const auto vectorAToBY =
        candidateBPtr[3 * j + 1] - candidateAPtr[3 * i + 1];
    ...
    // std::cout << "vectorAToBX: " << vectorAToBX << " vectorAToBY: " <<
    // vectorAToBY << " vectorAToBMax: " << vectorAToBMax << std::endl; If the
    // peaksPtr are coincident. Don't connect them.
    if (vectorNorm > 1e-6) {
      ...
      for (auto lm = 0; lm < numberPointsInLine; lm++) {
        ...
        const auto score =
            (vectorAToBNormX * mapX[idx] + vectorAToBNormY * mapY[idx]);
        // std::cout << "mX: " << mX << " mY: " << mY << " lm*vectorAToBXInLine:
        // " << lm*vectorAToBXInLine << std::endl; std::cout <<
        // "vectorAToBNormX: " << vectorAToBNormX << " maxX[idx]: " << mapX[idx]
        // << " vectorAToBNormY: " << vectorAToBNormY << " mapY: " << mapY[idx]
        // << std::endl; std::cout << "mX: " << mX << " mY: " << mY << " idx: "
        // << idx << " score: " << score << std::endl;
        if (score > interThreshold) {
          sum += score;
          count++;
        }
      }
      // std::cout << "count/T(numberPointsInLine): " << count << "/" <<
      // numberPointsInLine << "=" << count/T(numberPointsInLine) << std::endl;
      if (count / T(numberPointsInLine) > interMinAboveThreshold) {
        return sum / count;
      }
    }
...
//     Rectangle<int>& roi, int& diameter, int& partFirstNon0, int&
//     partLastNon0, const std::vector<int>& personVector, const T* const
//     peaksPtr, const int partInit, const int partEnd)
// {
//     try
//     {
    ...
    for (auto pairIndex = 0u; pairIndex < numberBodyPartPairs; pairIndex++) {
          ...
          if (numberBodyParts != 15) {
            for (auto i = 1; i <= numberPeaksB; i++) {
              ...
              if (!found) {
                ...
                peopleVector.emplace_back(
                    std::make_pair(rowVector, personScore));
              }
            }
          }
          // MPI
      ...
      else // if (numberPeaksA != 0 && numberPeaksB != 0)
      {
        ...
        if (heatMapPtr != nullptr) {
          ...
          const auto *mapY =
              heatMapPtr + (numberBodyPartsAndBkg + mapIdx[2 * pairIndex + 1]) *
                               heatMapOffset;
          ...
          for (auto i = 1; i <= numberPeaksA; i++) {
            ...
            for (auto j = 1; j <= numberPeaksB; j++) {
              // Initial PAF
              auto scoreAB = getScoreAB(i, j, candidateAPtr, candidateBPtr,
                                        mapX, mapY, heatMapSize, interThreshold,
  ...
  try {
    ...
    for (auto person = 0u; person < peopleVector.size(); person++) {
      ...
      if (!maximizePositives &&
          (numberBodyParts == 25 || numberBodyParts > 70)) {
        const auto currentCounter = personCounter;
...
//     template <typename T>
//     void connectDistanceStar(Array<T>& poseKeypoints, Array<T>& poseScores,
//     const T* const heatMapPtr,
//                              const T* const peaksPtr, const PoseModel
//                              poseModel, const Point<int>& heatMapSize, const
//                              int maxPeaks, const T scaleFactor, const
//                              unsigned int numberBodyParts, const unsigned int
//                              bodyPartPairsSize)
//     {
//         try
//         {
...
//                 const auto numberPeople =
//                 positiveIntRound(peaksPtr[rootNumberIndex]);
//                 poseKeypoints.reset({numberPeople, (int)numberBodyParts, 3},
//                 0); poseScores.reset(numberPeople, 0);
//                 // // 48 channels
//                 // const std::vector<float> AVERAGE{
//                 //     0.f, -2.76364f, -1.3345f, 0.f,   -1.95322f, 3.95679f,
//                 -1.20664f, 4.76543f,
//                 //     1.3345f, 0.f, 1.92318f, 3.96891f,   1.17999f, 4.7901f,
//                 0.f, 7.72201f,
//                 //     -0.795236f, 7.74017f, -0.723963f,   11.209f,
//                 -0.651316f, 15.6972f,
//                 //     0.764623f, 7.74869f, 0.70755f,   11.2307f,
//                 0.612832f, 15.7281f,
//                 //     -0.123134f, -3.43515f,   0.111775f, -3.42761f,
//                 //     -0.387066f, -3.16603f,   0.384038f, -3.15951f,
//                 //     0.344764f, 12.9666f, 0.624157f,   12.9057f,
//                 0.195454f, 12.565f,
//                 //     -1.06074f, 12.9951f, -1.2427f,   12.9309f,
//                 -0.800837f, 12.5845f};
//                 // const std::vector<float> SIGMA{
//                 //     3.39629f, 3.15605f, 3.16913f, 1.8234f,   5.82252f, 5.05674f,
//                 7.09876f, 6.64574f,
//                 //     3.16913f, 1.8234f, 5.79415f, 5.01424f,   7.03866f, 6.62427f,
//                 5.52593f, 6.75962f,
...
//                 const std::vector<float> AVERAGE{
//                     0.f, -6.55251f,
//                     0.f, -4.15062f, -1.48818f, -4.15506f,   -2.22408f,
//                     -0.312264f, -1.42204f, 0.588495f, 1.51044f,
//                     -4.14629f, 2.2113f, -0.312283f,   1.41081f, 0.612377f,
//                     -0.f, 3.41112f, -0.932306f, 3.45504f,
//                     -0.899812f,   6.79837f, -0.794223f, 11.4972f,
//                     0.919047f, 3.46442, 0.902314f,   6.81245f,
//                     0.79518f, 11.5132f, -0.243982f, -7.07925f,   0.28065f,
//                     -7.07398f, -0.792812f, -7.09374f,   0.810145f, -7.06958f,
//                     0.582387f, 7.46846f, 0.889349f,   7.40577f,
//                     0.465088f, 7.03969f, -0.96686f, 7.46148f,
//                     -1.20773f,   7.38834f, -0.762135f, 6.99575f};
//                 const std::vector<float> SIGMA{
//                     7.26789f, 9.70751f,
//                     6.29588f, 8.93472f, 6.97401f, 9.13746f,   7.49632f, 9.44757f,
//                     8.06695f, 9.97319f,
//                     6.99726f, 9.14608f, 7.50529f, 9.43568f,   8.05888f, 9.98207f,
//                     6.38929f, 9.29314f,
...
//                 const auto numberBodyPartsAndBkgAndPAFChannels =
//                 numberBodyParts + 1 + bodyPartPairsSize; const auto
//                 heatMapOffset = heatMapSize.area();
//                 // For each person
...
//                     poseKeypoints[{p,rootIndex,2}] =
//                     peaksPtr[rootXYSIndex+2];
//                     // For each body part
//                     for (auto bpOrig = 0 ; bpOrig < (int)numberBodyParts ;
//                     bpOrig++)
//                     {
//                         if (bpOrig != rootIndex)
//                         {
//                             // // 48 channels
//                             // const auto bpChannel = (bpOrig < rootIndex ?
//                             bpOrig : bpOrig-1);
//                             // 50 channels
//                             const auto bpChannel = bpOrig;
//                             // Get ideal distance
//                             const auto offsetIndex =
//                             numberBodyPartsAndBkgAndPAFChannels +
//                             2*bpChannel; const auto* mapX = heatMapPtr +
//                             offsetIndex * heatMapOffset; const auto* mapY =
//                             heatMapPtr + (offsetIndex+1) * heatMapOffset;
//                             const auto increaseRatio =
//                             scaleFactor*scaleDownFactor;
//                             // Set (x,y) coordinates from the distance
//                             const auto indexChannel = 2*bpChannel;
//                             // // Not refined method
//                             // const auto index =
//                             positiveIntRound(rootY/scaleFactor)*heatMapSize.x
//                                                 +
...
//                             // poseKeypoints[{p,bpOrig,0}] = rootX +
//                             neckPartDist.x;
//                             // poseKeypoints[{p,bpOrig,1}] = rootY +
//                             neckPartDist.y;
//                             // Refined method
...
//                             for (auto y = fastMax(0,
//                             positiveIntRound(rootY/scaleFactor) - constant);
//                                  y < fastMin(heatMapSize.y,
//                                  positiveIntRound(rootY/scaleFactor) +
//                                  constant+1) ; y++)
//                             {
//                                 for (auto x = fastMax(0,
//                                 positiveIntRound(rootX/scaleFactor) -
//                                 constant);
//                                      x < fastMin(heatMapSize.x,
//                                      positiveIntRound(rootX/scaleFactor) +
//                                      constant+1) ; x++)
//                                 {
...
//                             };
//                             neckPartDistRefined *=
//                             increaseRatio/counterRefinements; const auto
//                             partX = rootX + neckPartDistRefined.x; const auto
//                             partY = rootY + neckPartDistRefined.y;
...
//                                 0, fastMin(heatMapSize.x-1,
//                                 positiveIntRound(partX/scaleFactor)));
//                             const auto yCleaned = fastMax(
//                                 0, fastMin(heatMapSize.y-1,
//                                 positiveIntRound(partY/scaleFactor)));
//                             const auto partConfidence = heatMapPtr[
//                                 bpOrig * heatMapOffset +
//                                 yCleaned*heatMapSize.x + xCleaned];
//                             // If partConfidence is big enough, it means we
//                             are close to a keypoint if (partConfidence >
//                             T(0.05))
//                             {
//                                 const auto candidateNumberIndex =
//                                 bpOrig*(maxPeaks+1)*3; const auto
//                                 numberCandidates =
//                                 positiveIntRound(peaksPtr[candidateNumberIndex]);
//                                 int closestIndex = -1;
//                                 T closetValue =
//                                 std::numeric_limits<T>::max(); for (auto i =
//                                 0 ; i < numberCandidates ; i++)
//                                 {
//                                     const auto candidateXYSIndex =
//                                     candidateNumberIndex+3*(1+i); const auto
//                                     diffX =
//                                     partX-scaleFactor*peaksPtr[candidateXYSIndex];
//                                     const auto diffY =
//                                     partY-scaleFactor*peaksPtr[candidateXYSIndex+1];
//                                     const auto dist = (diffX*diffX +
//                                     diffY*diffY); if (closetValue > dist)
//                                     {
//                                         closetValue = dist;
...
//                                 if (closestIndex != -1)
//                                 {
//                                     poseKeypoints[{p,bpOrig,0}] =
//                                     scaleFactor*peaksPtr[closestIndex];
//                                     poseKeypoints[{p,bpOrig,1}] =
//                                     scaleFactor*peaksPtr[closestIndex+1];
//                                     // Set body part score
//                                     poseKeypoints[{p,bpOrig,2}] =
//                                     peaksPtr[closestIndex+2];
//                                 }
//                             }
//                             // Set poseScore
...
//     const std::vector<float> AVERAGE{
//         0.f, -6.55251f,
//         0.f, -4.15062f, -1.48818, -4.15506f,   -2.22408f, -0.312264f,
//         -1.42204f, 0.588495f, 1.51044f, -4.14629f, 2.2113f,
//         -0.312283f,   1.41081f, 0.612377f, -0.f, 3.41112f,
...
//     const std::vector<float> SIGMA{
//         7.26789f, 9.70751f,
//         6.29588f, 8.93472f, 6.97401f, 9.13746f,   7.49632f, 9.44757f, 8.06695f,
//         9.97319f,
//         6.99726f, 9.14608f, 7.50529f, 9.43568f,   8.05888f, 9.98207f, 6.38929f,
//         9.29314f,
//         6.71801f, 9.39271f, 8.00608f,   10.6141f, 10.3416f, 12.7812f,
//         6.69875f, 9.41407f, 8.01876f,   10.637f, 10.3475f, 12.7849f,
//         7.30923f, 9.7324f,   7.27886f, 9.73406f,
...
//         7.4655f, 8.25336f, 7.43958f,   8.26333f, 7.33667f, 7.97446f};
//     template <typename T>
//     std::array<T,3> regressPart(const Array<T>& person, const int rootIndex,
//     const int targetIndex,
//                                 const int scaleDownFactor, const T* const
//                                 heatMapPtr, const T* const peaksPtr, const
//                                 Point<int>& heatMapSize, const int maxPeaks,
//                                 const T scaleFactor, const unsigned int
//                                 numberBodyPartsAndBkgAndPAFChannels)
//     {
//         try
//         {
...
//                 const auto offsetIndex = numberBodyPartsAndBkgAndPAFChannels
//                 + indexChannel; const auto heatMapOffset =
//                 heatMapSize.area(); const auto* mapX = heatMapPtr +
//                 offsetIndex * heatMapOffset; const auto* mapY = heatMapPtr +
//                 (offsetIndex+1) * heatMapOffset; const auto increaseRatio =
//                 scaleFactor*scaleDownFactor;
//                 // // Not refined method
//                 // const auto index =
//                 positiveIntRound(rootY/scaleFactor)*heatMapSize.x
...
//                 auto counterRefinements = 0;
//                 // We must keep it inside the image size
//                 for (auto y = fastMax(0, positiveIntRound(rootY/scaleFactor)
//                 - constant);
//                      y < fastMin(heatMapSize.y,
//                      positiveIntRound(rootY/scaleFactor) + constant+1) ; y++)
//                 {
//                     for (auto x = fastMax(0,
//                     positiveIntRound(rootX/scaleFactor) - constant);
//                          x < fastMin(heatMapSize.x,
//                          positiveIntRound(rootX/scaleFactor) + constant+1) ;
//                          x++)
//                     {
//                         const auto index = y*heatMapSize.x + x;
//                         neckPartDistRefined.x += mapX[index];
...
//                 result[2] = T(0.0501);
//                 // Associate estimated keypoint with closest one
//                 const auto xCleaned = fastMax(0, fastMin(heatMapSize.x-1,
//                 positiveIntRound(partX/scaleFactor))); const auto yCleaned =
//                 fastMax(0, fastMin(heatMapSize.y-1,
//                 positiveIntRound(partY/scaleFactor))); const auto
//                 partConfidence = heatMapPtr[
//                     targetIndex * heatMapOffset + yCleaned*heatMapSize.x +
//                     xCleaned];
//                 // If partConfidence is big enough, it means we are close to
//                 a keypoint if (partConfidence > T(0.05))
//                 {
//                     const auto candidateNumberIndex =
//                     targetIndex*(maxPeaks+1)*3; const auto numberCandidates =
//                     positiveIntRound(peaksPtr[candidateNumberIndex]); int
//                     closestIndex = -1; T closetValue =
//                     std::numeric_limits<T>::max(); for (auto i = 0 ; i <
//                     numberCandidates ; i++)
//                     {
//                         const auto candidateXYSIndex =
//                         candidateNumberIndex+3*(1+i); const auto diffX =
//                         partX-scaleFactor*peaksPtr[candidateXYSIndex]; const
//                         auto diffY =
//                         partY-scaleFactor*peaksPtr[candidateXYSIndex+1];
...
//     template <typename T>
//     void connectDistanceMultiStar(Array<T>& poseKeypoints, Array<T>&
//     poseScores, const T* const heatMapPtr,
//                                   const T* const peaksPtr, const PoseModel
//                                   poseModel, const Point<int>& heatMapSize,
//                                   const int maxPeaks, const T scaleFactor,
//                                   const unsigned int numberBodyParts, const
//                                   unsigned int bodyPartPairsSize)
//     {
//         try
//         {
//             // poseKeypoints from neck-part distances
...
//                 const std::vector<int> keypointsSize = {(int)numberBodyParts,
//                 3};
...
//                     1, 8, 0, 2,5,9,12, 3,6,10,13, 15,16, 4,7,11,14, 17,18,
//                     19,22,20,23,21,24};
//                 const auto numberBodyPartsAndBkgAndPAFChannels =
//                 numberBodyParts + 1 + bodyPartPairsSize; const auto
//                 scaleDownFactor = 8; for (auto index = 0u ; index <
//                 numberBodyParts ; index++)
//                 {
//                     const auto targetIndex = MAPPING[index];
//                     // Get all candidate keypoints
//                     const auto partNumberIndex = targetIndex*(maxPeaks+1)*3;
//                     const auto numberPartParts =
//                     positiveIntRound(peaksPtr[partNumberIndex]);
//                     std::vector<std::array<T, 3>>
//                     currentPartCandidates(numberPartParts); for (auto i = 0u
//                     ; i < currentPartCandidates.size() ; i++)
//                     {
//                         const auto baseIndex = partNumberIndex+3*(i+1);
//                         currentPartCandidates[i][0] =
//                         scaleFactor*peaksPtr[baseIndex];
//                         currentPartCandidates[i][1] =
//                         scaleFactor*peaksPtr[baseIndex+1];
...
//                     {
//                         // Estimate new body part w.r.t. each
//                         already-detected body part for (auto rootMapIndex =
//                         0u ; rootMapIndex < index ; rootMapIndex++)
//                         {
//                             const auto rootIndex = MAPPING[rootMapIndex];
//                             if (person[{rootIndex,2}] > T(0.0501))
//                             {
//                                 const auto result = regressPart(
//                                     person, rootIndex, targetIndex,
//                                     scaleDownFactor, heatMapPtr, peaksPtr,
//                                     heatMapSize, maxPeaks, scaleFactor,
//                                     numberBodyPartsAndBkgAndPAFChannels);
...
//                     for (auto p = 0u ; p < currentPartCandidates.size() ;
//                     p++)
//                     {
//                         poseKeypointsTemp[currentSize+p] =
//                         Array<T>(keypointsSize, 0.f); const auto baseIndex =
//                         3*targetIndex;
//                         poseKeypointsTemp[currentSize+p][baseIndex  ] =
//                         currentPartCandidates[p][0];
//                         poseKeypointsTemp[currentSize+p][baseIndex+1] =
//                         currentPartCandidates[p][1];
//                         poseKeypointsTemp[currentSize+p][baseIndex+2] =
//                         currentPartCandidates[p][2];
//                     }

// }
//                 }
//                 // poseKeypoints: Reformat poseKeypointsTemp as poseKeypoints
//                 poseKeypoints.reset({(int)poseKeypointsTemp.size(),
//                 (int)numberBodyParts, 3}, 0);
//                 poseScores.reset(poseKeypoints.getSize(0), 0.f);
//                 const auto keypointArea =
//                 poseKeypoints.getSize(1)*poseKeypoints.getSize(2); for (auto
//                 p = 0 ; p < poseKeypoints.getSize(0) ; p++)
//                 {
//                     const auto pIndex = p*keypointArea;
//                     for (auto part = 0 ; part < poseKeypoints.getSize(1) ;
//                     part++)
//                     {
//                         const auto baseIndexTemp = 3*part;
//                         const auto baseIndex = pIndex+baseIndexTemp;
//                         poseKeypoints[baseIndex  ] =
//                         poseKeypointsTemp[p][baseIndexTemp];
//                         poseKeypoints[baseIndex+1] =
//                         poseKeypointsTemp[p][baseIndexTemp+1];
//                         poseKeypoints[baseIndex+2] =
//                         poseKeypointsTemp[p][baseIndexTemp+2];
//                         // Set poseScore
...
void connectBodyPartsCpu(Array<T> &poseKeypoints, Array<T> &poseScores,
                         ...
                         const T scaleFactor, const bool maximizePositives) {
  try {
    // std::cout << "---------------" << " dubug body part connect " <<
    // "-------" << std::endl; std::cout << "heatMapSize: " << heatMapSize.x <<
    // " " << heatMapSize.y << std::endl; std::cout << "maxPeaks: " << maxPeaks
    // << std::endl; std::cout << "interMinAboveThreshold: " <<
    // interMinAboveThreshold << std::endl; std::cout << "interThreshold: " <<
    // interThreshold << std::endl; std::cout << "minSubsetCnt: " <<
    // minSubsetCnt << std::endl; std::cout << "minSubsetScore: " <<
    // minSubsetScore << std::endl; std::cout << "scaleFactor: " << scaleFactor
    // << std::endl; std::cout << "maximizePositive: " << maximizePositives <<
    // std::endl; Parts Connection
    const auto &bodyPartPairs = std::vector<unsigned int>{
        1,  8,  1, 2,  1,  5,  2,  3,  3,  4,  5,  6,  6,  7,  8,  9,  9,  10,
        10, 11, 8, 12, 12, 13, 13, 14, 1,  0,  0,  15, 15, 17, 0,  16, 16, 18,
        2,  17, 5, 18, 14, 19, 19, 20, 14, 21, 11, 22, 22, 23, 11, 24};
    ...
    //     std::cout << "people number " << i << std::endl;
    //     for(int j=0;j<peopleVector[i].first.size();j++) {
    //         std::cout << peopleVector[i].first[j] << " ";
    //     }
    //     std::cout << peopleVector[i].second << std::endl;
    // }
    // Delete people below the following thresholds:
    // a) minSubsetCnt: removed if less than minSubsetCnt body parts
    // b) minSubsetScore: removed if global score smaller than this
    ...
    //                                          numberBodyParts,
    //                                          bodyPartPairs.size());
    //                 connectDistanceStar(poseKeypoints, poseScores,
    //                 heatMapPtr, peaksPtr, poseModel, heatMapSize,
    //                                     maxPeaks, scaleFactor,
    //                                     numberBodyParts,
    //                                     bodyPartPairs.size());
