 ...
 * data container, analogous to cv::Mat in OpenCV, Tensor in Torch/TensorFlow or
 * Blob in Caffe. It wraps a cv::Mat and a std::shared_ptr, both of them
 * pointing to the same raw data. I.e. they both share the same memory, so we
 * can read and modify this data in both formats with no performance impact.
...
class Array {
 public:
   ...
   */
  inline bool empty() const { return (mVolume == 0); }
   ...
   */
  inline std::vector<int> getSize() const { return mSize; }
   ...
   * @return A std::stringwith the size of each dimension. If no memory has been
   * allocated, it will return an empty string.
   */
   ...
   */
  inline size_t getNumberDimensions() const { return mSize.size(); }

  /**
   * Return the total number of elements allocated, equivalent to multiply all
   * the components from getSize(). E.g., for a Array<T> of size = {2,5,3}, the
   * volume or total number of elements is: 2x5x3 = 30.
   * @return The total volume of the allocated data. If no memory is allocated,
   * it returns 0.
   */
  inline size_t getVolume() const { return mVolume; }

  /**
   * Similar to getVolume(), but in this case it just returns the volume between
   * the desired dimensions. E.g., for a Array<T> of size = {2,5,3}, the volume
   * or total number of elements for getVolume(1,2) is 5x3 = 15.
   ...
   * E.g., given and Array<T> of size 5x3, getStride() would return the
   ...
   * @return A editable reference to the data on the desired index location.
   */
  inline T& at(const int index) { return commonAt(index); }
   ...
   * @return A non-editable reference to the data on the desired index location.
   */
  inline const T& at(const int index) const { return commonAt(index); }
   ...
   * The format is: values separated by a space, and a enter for each dimension.
   * E.g., For the Array{2, 2, 3}, it will print: Array<T>::toString(): x1 x2 x3
   * x4 x5 x6
   *
   * x7 x8 x9
   * x10 x11 x12
 ...
 private:
  ...
  /**
   * Similar to getIndex(const std::vector<int>& indexes) const, but used for
  ...
  /**
   * Auxiliar function that both at(const int index) and at(const int index)
   * const use.
   * @param index The desired memory location.
   * @return A non-editable reference to the data on the desired index location.
   */
  ...
  void resetAuxiliary(const std::vector<int>& sizes,
